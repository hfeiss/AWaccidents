...
def histogram(ax, data, saveas, title, color,
              labels=None, bins=None, remove_spines=False, tag=None):
    ...
    if title == 'Document Lemma Count':
        ax.set_xticks(np.arange(0, np.max(data),
                                np.max(data)/(len(labels) - 0.9)))
...
if __name__ == "__main__":
    ...
    histogram(ax, data, 'dates', 'Accidents Over Time', color='#047495',
    ...
    histogram(ax[0], level, 'level_diff_death_2',
              'Accidents vs. Water Level', color='#047495',
              labels=lev_labels, bins=4, remove_spines=True,
              tag='Non Fatal')
    histogram(ax[0], level_death, 'level_diff_death_2',
              'Accidents vs. Water Level', color='#980002',
              labels=lev_labels, bins=4, remove_spines=True,
              tag='Fatal',)
    histogram(ax[1], diff, 'level_diff_death_2',
              'Accidents vs. River Difficulty', color='#047495',
              labels=diff_labels, bins=5, remove_spines=True,
              tag='Non Fatal')
    histogram(ax[1], diff_death, 'level_diff_death_2',
              'Accidents vs. River Difficulty', color='#980002',
              labels=diff_labels, bins=5, remove_spines=True,
              tag='Fatal')
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    histogram(ax, data, 'description_len_death',
              'Document Lemma Count', color='#047495',
              labels=labels, bins=50, remove_spines=True,
              tag='Non Fatal')
    histogram(ax, death_data, 'description_len_death',
              'Document Lemma Count', color='#980002',
              labels=labels, bins=50, remove_spines=True,
              tag='Fatal')
    ...
    fig, ax = plt.subplots(1, 2, figsize=(10, 6))
    histogram(ax[0], exper, 'exper_age_death',
              'Accidents vs. Experience Level', color='#047495',
              labels=exper_labels, bins=4, remove_spines=True,
              tag='Non Fatal',)
    histogram(ax[0], exper_death, 'exper_age_death',
              'Accidents vs. Experience Level', color='#980002',
              labels=exper_labels, bins=4, remove_spines=True,
              tag='Fatal')
    histogram(ax[1], age, 'exper_age_death',
              'Accidents vs. Victim Age', color='#047495',
              remove_spines=True, tag='Non Fatal')
    histogram(ax[1], age_death, 'exper_age_death',
              'Accidents vs. Victim Age', color='#980002',
              remove_spines=True, tag='Fatal')
